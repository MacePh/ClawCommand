  ...
  <div class="shell">
    ...
    <section class="hero-grid">
      <div class="panel glass">
        ...
        <div class="activity-feed" id="activity-panel">Loading...</div>
      </div>
      <div class="stack-panel">
        <div class="panel glass">
          ...
          <div class="status-grid" id="command-status-panel">Loading...</div>
        </div>
        <div class="panel glass">
          <div class="panel-header">
            <div class="panel-title">
          ...
          <div class="memory-content" id="memory-panel">Loading...</div>
        </div>
        <div class="panel glass">
          <div class="panel-header">
            <div class="panel-title">
          ...
          <div class="command-input-wrapper">
            <input id="queue-task-input" class="command-input" placeholder="Enter task command..." />
    ...
    <section class="main-grid">
      <div class="panel glass wide">
        ...
        <div class="kanban" id="kanban">Loading...</div>
      </div>
      <div class="panel glass">
        <div class="panel-header">
          <div class="panel-title">
        ...
        <pre id="status-output" class="terminal-output">Loading...</pre>
      </div>
      <div class="panel glass">
        <div class="panel-header">
          <div class="panel-title">
        ...
        <div id="sessions" class="sessions-grid">Loading...</div>
      </div>
    </section>
  </div>
`
...
async function loadStatus() {
  ...
  try {
    ...
    updateIndicatorState('gateway-pill', gwState.includes('running') ? 'online' : 'error', 'Gateway')

    const tgState = data.parsed.telegramState.toLowerCase()
    updateIndicatorState('telegram-pill', tgState.includes('running') ? 'online' : 'error', 'Telegram')
  } catch (err) {
  ...
  try {
    ...
    panel.innerHTML = `
      ...
      <div class="status-item">
        ...
        <div class="status-item-value">${data.status}</div>
      </div>
    `
  } catch (err) {
...
async function loadQueueDepth() {
  try {
    ...
    if (r > 0) {
      updateIndicatorState('queue-pill', 'online', `Queue: ${r}`)
    } else if (p > 0) {
      updateIndicatorState('queue-pill', 'warning', `Queue: ${p}`)
    } else {
